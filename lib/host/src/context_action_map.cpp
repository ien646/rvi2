...
namespace rvi::host
{
    ...
    void i_call(const parsed_stmt& line, std::vector<definition_inst>& result)
    {
        expect_argc(line.args, 1, cmd_type::CALL);
        std::string def_name = line.args[0];
        result.push_back(instruction_generator::execute_definition(def_name));
    }
